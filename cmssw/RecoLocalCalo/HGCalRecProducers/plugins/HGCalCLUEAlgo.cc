...
void HGCalCLUEAlgoT<T, STRATEGY>::calculateLocalDensity(const T& lt,
  ...
  for (unsigned int i = 0; i < numberOfCells; i++) {
    ...
    // if (cellsOnLayer.dim1[i] < 1.81 && cellsOnLayer.dim1[i] > 1.8) {
    //   std::cout << "There are " << num_boxes << " bins and " << num_cells << " hits in the searching box of the cell " << i << " with eta " << cellsOnLayer.dim1[i] << ", phi " << cellsOnLayer.dim2[i] << ", in layer " << layerId << ".\n" << std::flush;
    // }
