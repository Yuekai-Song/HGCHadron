...
void HGVHistoProducerAlgo::bookClusterHistos_LCtoCP_association(DQMStore::IBooker& ibook,
  ...
  for (unsigned ilayer = 0; ilayer < 2 * layers; ++ilayer) {
    ...
    for (auto const& score : VScoreCutCPtoLC_) {
      ...
      histograms.h_responce_sim2reco_perlayer[ilayer].push_back(
        ibook.book1D("Responce_sim2reco_perlayer" + istr1 + "_cut" + s,
                        "Responce with SimToReco for layer " + istr2 + " with cut " + s,
                        50,
                        0,
                        5));
    }
    for (auto const& score : VScoreCutLCtoCP_) {
      std::string s = to_p_string(score);
      histograms.h_responce_reco2sim_perlayer[ilayer].push_back(
        ibook.book1D("Responce_reco2sim_perlayer" + istr1 + "_cut" + s,
                        "Responce with RecoToSim for layer " + istr2 + " with cut " + s,
                        50,
                        0,
                        5));
    }
  ...
  for (auto const& score : VScoreCutCPtoLC_) {
    ...
    histograms.h_responce_sim2reco_all.push_back(
        ibook.book1D("Responce_sim2reco_cut" + s, "Responce with SimToReco with cut " + s, 100, 0.8, 1.6));
    histograms.h_responce_sim2reco_e.push_back(
        ibook.book1D("Responce_E_sim2reco_cut" + s, "Responce for CE-E CP with SimToReco with cut " + s, 100, 0.8, 1.6));
    histograms.h_responce_sim2reco_h.push_back(
        ibook.book1D("Responce_H_sim2reco_cut" + s, "Responce for CE-H CP with SimToReco with cut " + s, 100, 0.8, 1.6));
  }
  for (auto const& score : VScoreCutLCtoCP_) {
    std::string s = to_p_string(score);
    ...
    histograms.h_responce_reco2sim_all.push_back(
        ibook.book1D("Responce_reco2sim_cut" + s, "Responce with RecoToSim with cut " + s, 100, 0.8, 1.6));
    histograms.h_responce_reco2sim_e.push_back(
        ibook.book1D("Responce_E_reco2sim_cut" + s, "Responce for CE-E CP with RecoToSim with cut " + s, 100, 0.8, 1.6));
    histograms.h_responce_reco2sim_h.push_back(
        ibook.book1D("Responce_H_reco2sim_cut" + s, "Responce for CE-H CP with RecoToSim with cut " + s, 100, 0.8, 1.6));
  }
  //---------------------------------------------------------------------------------------------------------------------------
...
void HGVHistoProducerAlgo::bookResponseHistos(DQMStore::IBooker& ibook, Histograms& histograms) {
  ...
  histograms.h_response_simtrackstersFromCP2caloparticle = ibook.book2D("Responce_simtrackstersFromCP2caloparticle",
                                                        ...
                                                        0,
                                                        1.5);
  histograms.h_response_trackstersCLUE3DHigh2caloparticle = ibook.book2D("Responce_trackstersCLUE3DHigh2caloparticle",
                                                        ...
                                                        0,
                                                        1.5);
  histograms.h_response_ticlcandidate2caloparticle = ibook.book2D("Responce_ticlcandidate2caloparticle",
                                                        ...
                                                        0,
                                                        1.5);
  histograms.h_response_layerClusters2caloparticle = ibook.book2D("Responce_layerClusters2caloparticle",
                                                        ...
                                                        0,
                                                        1.5);
  histograms.h_response_reg_trackstersCLUE3DHigh2caloparticle = ibook.book2D("Responce_trackstersCLUE3DHigh2caloparticle_regressed",
                                                        "Response of trackstersCLUE3DHigh to caloparticle,regressed;caloparticle energy [GeV];response",
                                                        nintEne_,
                                                        minEne_,
                                                        maxEne_,
                                                        150,
                                                        0,
                                                        1.5);
  histograms.h_response_reg_ticlcandidate2caloparticle = ibook.book2D("Responce_ticlcandidate2caloparticle_regressed",
                                                        ...
                                                        0,
                                                        1.5);
  histograms.h_response_reg_layerClusters2caloparticle = ibook.book2D("Responce_layerClusters2caloparticle_regressed",
                                                        "Response of layerClusters to caloparticle,regressed;caloparticle energy [GeV];response",
                                                        nintEne_,
                                                        minEne_,
                                                        maxEne_,
                                                        150,
...
void HGVHistoProducerAlgo::fill_response_histos(
  ...
  //   cout << "Gen particle ID: " << genParticle.pdgId() << ", energy: " << genParticle.energy() << endl;
  // }
  for (const auto& cpId : cPSelectedIndices) {
    ...
    for (const auto& simCluster : cP[cpId].simClusters()) {
      for (const auto& it_haf : simCluster->hits_and_fractions()) {
        ...
        if (itcheck != hitMap.end()) {
          ...
          // cout << "fraction: " << it_haf.second << endl;
    ...
    for (unsigned int simTracksterIndex = 0; simTracksterIndex < nSimTracksters; ++simTracksterIndex) {
      ...
      // cout << "Number of gen particles: " << cP[cpId].genParticles().size() << endl;
      // for (const auto& genParticle : cP[cpId].genParticles()) {
      //   cout << "Gen particle ID: " << genParticle->pdgId() << ", energy: " << genParticle->energy() << endl;
      // }
      // cout << "CP energy " << cP[cpId].energy() << " vs CP sim Energy " << cP[cpId].simEnergy() << " vs CP hits franction Energy " << cPEnergy << " vs CP hits energy " << cPEnergy_all << " vs ST regressed energy " << simTrackster.regressed_energy() << " vs ST raw Energy " << simTrackster.raw_energy() << endl;
      float trackstersEnergy = 0, trackstersEnergy_regressed = 0;
      for (unsigned int i = 0; i < simTrackstersFromCPsToTrackstersByLCsMap[simTracksterIndex].size(); ++i) {
        ...
        if (score < 1) {
          ...
          // cout << "test different ways to get the same trackster: " << a->raw_energy() << " vs " << b.raw_energy() << endl;
      ...
      histograms.h_response_reg_trackstersCLUE3DHigh2caloparticle->Fill(cPEnergy, trackstersEnergy_regressed / cPEnergy);
      ...
      if (!ts_vec.empty()) {
        ...
        auto candRawEnergy = TICLCandidates[cand_idx].rawEnergy();
        auto candEnergy = TICLCandidates[cand_idx].energy();
        // cout << "test different ways to get the same ticl candidate: " << candEnergy << " vs " << candRawEnergy << endl;
...
void HGVHistoProducerAlgo::layerClusters_to_CaloParticles(
  ...
  for (unsigned int lcId = 0; lcId < nLayerClusters; ++lcId) {
    ...
    //   const HGCRecHit* hit = &(hits[itcheck->second]);
    //   if (rhPair.second < 1) {
    //     std::cout << "LC: " << lcId << std::endl;
    //     std::cout << "hitId: " << rhPair.first.rawId() << ", fraction: " << rhPair.second
  ...
  for (const auto& cpId : cPSelectedIndices) {
    ...
    for (unsigned int layerId = 0; layerId < layers * 2; ++layerId) {
      ...
      for (const auto& lcPair : lcs) {
        ...
        //       std::cout << "hitId: " << rhPair.first << ", fraction: " << rhPair.second.first
        //                 << ", energy: " << rhPair.second.second << std::endl;
        //   }
        //   std::cout << "LC: " << lcPair.first.index() << std::endl;
        //   std::cout << "At layer" << layerId << ", shared energy: " << lcPair.second.first << ", S2R: " << lcPair.second.second << ", R2S: " << lc_linked->second << std::endl;
        //   std::cout << "hits in LC: " << std::endl;
        //   for (auto const& rhPair: clusters[lcPair.first.index()].hitsAndFractions()) {
        //     std::unordered_map<DetId, const unsigned int>::const_iterator itcheck = hitMap.find(rhPair.first);
        //     const HGCRecHit* hit = &(hits[itcheck->second]);
        //     std::cout << "hitId: " << rhPair.first.rawId() << ", fraction: " << rhPair.second << ", energy: " << hit->energy() << std::endl;
        //   }
        // }
      }

      for (unsigned i = 0; i < VScoreCutCPtoLC_.size(); i++)
        histograms.h_responce_sim2reco_perlayer.at(layerId)[i]->Fill(vec_sum_lc_sim2reco_energy_perlayer[layerId][i] / cPEnergyOnLayer[layerId]);
      for (unsigned i = 0; i < VScoreCutLCtoCP_.size(); i++)
        histograms.h_responce_reco2sim_perlayer.at(layerId)[i]->Fill(vec_sum_lc_reco2sim_energy_perlayer[layerId][i] / cPEnergyOnLayer[layerId]);
    ...
    for (unsigned i = 0; i < VScoreCutCPtoLC_.size(); i++) {
      histograms.h_responce_sim2reco_all[i]->Fill(vec_sum_lc_sim2reco_energy[i] / e_tot);
      if (e_ee / e_tot >= 0.9)
        histograms.h_responce_sim2reco_e[i]->Fill(vec_sum_lc_sim2reco_energy[i] / e_tot);
      if (e_ee / e_tot < 0.1)
        histograms.h_responce_sim2reco_h[i]->Fill(vec_sum_lc_sim2reco_energy[i] / e_tot);
    }
    for (unsigned i = 0; i < VScoreCutLCtoCP_.size(); i++) {
      histograms.h_responce_reco2sim_all[i]->Fill(vec_sum_lc_reco2sim_energy[i] / e_tot);
      if (e_ee / e_tot >= 0.9)
        histograms.h_responce_reco2sim_e[i]->Fill(vec_sum_lc_reco2sim_energy[i] / e_tot);
      if (e_ee / e_tot < 0.1)
        histograms.h_responce_reco2sim_h[i]->Fill(vec_sum_lc_reco2sim_energy[i] / e_tot);
    }
    // if (e_bh / e_tot > 0.5) {
    // std::cout << "with " << e_bh / e_tot << " energy in BH: event: " << cP[cpId].eventId().event() << "\n" << std::flush;
    // }
  }
  // cout << "CP energy from layerclusters: " << CP_energy_tot << std::endl;
  ...
  if (response_booked_) {
    histograms.h_response_reg_layerClusters2caloparticle->Fill(CP_energy_tot, lc_tot_energy_in_CP / CP_energy_tot);
    histograms.h_response_layerClusters2caloparticle->Fill(CP_energy_tot, lc_tot_energy / CP_energy_tot);
  }
  // cout << "Total energy in layerclusters: " << lc_tot_energy << std::endl;
  // cout << "Total energy in layerclusters associated to CP: " << lc_tot_energy_in_CP << std::endl;
  for (unsigned int layerId = 0; layerId < layers * 2; ++layerId) {
