...
def main(opts):
    ...
    if opts.ticlv == 5:
        ticlVersion = 5
    opts.files = [os.path.join(opts.dir, f) for f in opts.files]
    opts.outputDir = [os.path.join(opts.dir, d) for d in opts.outputDir]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=str, default="electron_noPU",
    ...
    parser.add_argument("-o", "--outputDir", type=str, default=["plots","plots2"], nargs="+",
                        help="Plot output directories (default: 'plots'")
    parser.add_argument("--subdirprefix", type=str, default=["plots","plots2"], nargs="+",
    ...
    for f in opts.files:
        if not os.path.exists(os.path.join(opts.dir, f)):
